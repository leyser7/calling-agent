...
export class StreamSession {
  ...
  public async setupSessionAndPromptStart(voiceId: string): Promise<void> {
  ...
  public async setupSystemPrompt(
    ...
    systemPromptContent: string): Promise<void> {
  ...
  public async setupStartAudio(
    ...
    this.client.setupStartAudioEvent(this.sessionId, voiceId, audioConfig);
  }
  ...
  private async processAudioQueue() {
    ...
    try {
      ...
      while (this.audioBufferQueue.length > 0 && processedChunks < this.maxChunksPerBatch && this.isActive) {
...
export class NovaSonicBidirectionalStreamClient {
  ...
  public setupPromptStartEvent(sessionId: string, voiceId: string): void {
    console.log(`Setting up prompt start event for session ${sessionId} with voiceId: ${voiceId}`);
    const session = this.activeSessions.get(sessionId);
    if (!session) return;
    
    const audioOutputConfig = { ...DefaultAudioOutputConfiguration, voiceId };
  ...
  public setupStartAudioEvent(
    ...
    console.log(`Setting up startAudioContent event for session ${sessionId} with voiceId: ${voiceId}`);
    const session = this.activeSessions.get(sessionId);
    if (!session) return;
